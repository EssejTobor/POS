...
class TestAddThoughtCommand(unittest.TestCase):
    ...
    def test_add_thought_input_parser(self):
        ...
        input_str = "TestGoal-Test Title-Test description --parent abc123 --link inspired-by"
        parsed = AddThoughtInput.parse_input(input_str)
        self.assertEqual(parsed.goal, "TestGoal")
        self.assertEqual(parsed.title, "Test Title")
        self.assertEqual(parsed.description, "Test description")
        ...
        # Invalid link type
        with self.assertRaises(ValueError):
            AddThoughtInput.parse_input("TestGoal-Test Title-Test description --parent abc123 --link invalid-type")
    ...
    def test_add_thought_command(self):
        ...
        with redirect_stdout(captured_output):
            self.cli.onecmd(f"add_thought TestGoal-Child Thought-This references the parent --parent {parent_id}")
        ...
        with redirect_stdout(captured_output):
            self.cli.onecmd(f"add_thought TestGoal-Inspired Thought-This was inspired by the original --parent {parent_id} --link inspired-by")
