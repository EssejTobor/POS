...
class TestUnifiedAddCommand(unittest.TestCase):
    ...
    def test_add_thought_with_linking(self):
        ...
        with redirect_stdout(captured_output):
            self.cli.onecmd(
                f"add TestGoal-th-MED-Linked Thought-This references a task --link-to {task.id}"
            )
    ...
    def test_add_thought_with_custom_link_type(self):
        ...
        with redirect_stdout(captured_output):
            self.cli.onecmd(
                f"add TestGoal-th-MED-Evolution-This evolves the original --link-to {thought1.id} --link-type evolves-from"
            )
