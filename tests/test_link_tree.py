...
class TestLinkTree(unittest.TestCase):
    ...
    def test_link_tree_with_root(self):
        ...
        self.assertIn(
            f"Displaying relationship tree for: {self.thought1.title}", output
        )
    ...
    def test_missing_item_cleanup(self):
        ...
        self.assertGreaterEqual(
            output.count(f"Item not found: {missing_thought.id}"), 2
        )
        self.assertNotIn(
            f"{missing_thought.id}[/dim cyan] [dim](cycle reference)", output
        )
