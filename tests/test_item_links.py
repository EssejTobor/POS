...
class TestItemLinks(unittest.TestCase):
    ...
    def test_add_link(self):
        ...
        self.assertFalse(
            result, "Adding link with non-existent source should return False"
        )

        # Try to add a link with non-existent target_id
        result = self.db.add_link(self.item1.id, "non-existent-id")
        self.assertFalse(
            result, "Adding link with non-existent target should return False"
        )
    ...
    def test_get_links(self):
        ...
        self.assertEqual(
            len(links1["outgoing"]), 1, "Item1 should have one outgoing link"
        )
        self.assertEqual(
            len(links1["incoming"]), 1, "Item1 should have one incoming link"
        )
        self.assertEqual(
            links1["outgoing"][0]["target_id"],
            self.item2.id,
            "Outgoing link should point to item2",
        )
        self.assertEqual(
            links1["outgoing"][0]["link_type"],
            "references",
            "Link type should be 'references'",
        )
        self.assertEqual(
            links1["incoming"][0]["source_id"],
            self.item2.id,
            "Incoming link should come from item2",
        )
        self.assertEqual(
            links1["incoming"][0]["link_type"],
            "evolves-from",
            "Link type should be 'evolves-from'",
        )

        # Get links for item2
        links2 = self.db.get_links(self.item2.id)
        self.assertEqual(
            len(links2["outgoing"]), 1, "Item2 should have one outgoing link"
        )
        self.assertEqual(
            len(links2["incoming"]), 1, "Item2 should have one incoming link"
        )

        # Get links for non-existent item
        links_none = self.db.get_links("non-existent-id")
        self.assertEqual(
            len(links_none["outgoing"]),
            0,
            "Non-existent item should have no outgoing links",
        )
        self.assertEqual(
            len(links_none["incoming"]),
            0,
            "Non-existent item should have no incoming links",
        )
