...
def main():
    ...
    # Get the thought ID
    thought_id = [
        item.id for item in cli.work_system.get_items_by_type(ItemType.THOUGHT)
    ][0]
    ...
    cli.onecmd(
        f"add_thought TestProject-Followup Idea-This builds on my first thought --parent {thought_id}"
    )
    ...
    for thought in thoughts:
        ...
        if links["outgoing"]:
            ...
            for link in links["outgoing"]:
                print(
                    f"    -> {link['target_id']} - {link['title']} (Type: {link['link_type']})"
                )
        ...
        if links["incoming"]:
            ...
            for link in links["incoming"]:
                print(
                    f"    <- {link['source_id']} - {link['title']} (Type: {link['link_type']})"
                )
