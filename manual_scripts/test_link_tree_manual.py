...
def main():
    ...
    cli.onecmd(
        "add ProjectX-th-HI-Initial Concept-This is the starting point for the project"
    )
    ...
    if not initial_thought_id or not task_id:
        print(
            "Error: Could not find test items. Make sure they were created successfully."
        )
        return

    # Create a reference link
    print("Creating a reference link from thought to task...")
    ...
    cli.onecmd(
        f"add ProjectX-th-MED-Refined Concept-This builds on the initial concept --link-to {initial_thought_id} --link-type evolves-from"
    )
    ...
    cli.onecmd(
        f"add ProjectX-th-LOW-Alternative Approach-This takes a different direction --link-to {evolved_thought_id} --link-type inspired-by"
    )

    # Create a fourth thought that branches off in a new direction
    print("Creating a branching thought...")
    cli.onecmd(
        f"add ProjectX-th-HI-Implementation Plan-Concrete steps for implementation --link-to {evolved_thought_id} --link-type evolves-from"
    )
    ...
    cli.onecmd(
        f"add ProjectX-th-MED-Technical Limitations-Constraints to consider --link-to {implementation_thought_id} --link-type references"
    )
