...
class MigrationManager:
    def __init__(
        self, json_path: str = "work_items.json", db_path: str = "work_items.db"
    ):
    ...
    def validate_json_data(self, data: Dict[str, Any]) -> List[str]:
        ...
        for item_id, item_data in items_dict.items():
            try:
                ...
                if missing_fields:
                    errors.append(
                        f"Item {item_id} missing fields: {', '.join(missing_fields)}"
                    )
                    continue
                ...
                if "item_type" in item_data:
                    try:
                        ...
                        errors.append(
                            f"Item {item_id} has invalid item_type: {item_data['item_type']}"
                        )
                ...
                if "priority" in item_data:
                    try:
                        ...
                        errors.append(
                            f"Item {item_id} has invalid priority: {item_data['priority']}"
                        )
                ...
                if "status" in item_data:
                    try:
                        ...
                        errors.append(
                            f"Item {item_id} has invalid status: {item_data['status']}"
                        )
                ...
                for timestamp_field in ["created_at", "updated_at"]:
                    if timestamp_field in item_data:
                        try:
                            ...
                            errors.append(
                                f"Item {item_id} has invalid {timestamp_field}"
                            )
            ...
            for item_data in items_dict.values():
                try:
                    ...
                    migration_errors.append(
                        f"Error processing item {item_data.get('id', 'unknown')}: {str(e)}"
                    )
